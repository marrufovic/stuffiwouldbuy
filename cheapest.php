...
$sql = "SELECT * FROM products WHERE approved='1' ORDER BY price LIMIT $lowlim, $highlim";
...
{
	$sql = "SELECT * FROM products WHERE approved='1' AND category LIKE '%$cat%' ORDER BY price LIMIT $lowlim, $highlim";
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "good"){
				location.href = 'cheapest.php?=<?php echo $_SESSION['category']?>';
			} else {
...
<div id="wrapper">
 <div id="adTop">
  ...
  <option value="3" selected="selected">Cheapest</option>
