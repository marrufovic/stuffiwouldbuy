...
$sql = "SELECT * FROM products WHERE approved='1' ORDER BY RAND() DESC LIMIT $lowlim, $highlim";
...
{
	$sql = "SELECT * FROM products WHERE approved='1' AND category LIKE '%$cat%' ORDER BY RAND() DESC LIMIT $lowlim, $highlim";
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "good"){
				location.href = 'random.php?=<?php echo $_SESSION['category']?>';
			} else {
...
<div id="wrapper">
 <div id="adTop">
  ...
  <option value="5" selected="selected">Surprise Me</option>
  </select>
  </form>
  </div>
 <div id="shadow">
