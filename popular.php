...
$sql = "SELECT * FROM products WHERE approved='1' ORDER BY saves DESC LIMIT $lowlim, $highlim";
...
{
	$sql = "SELECT * FROM products WHERE approved='1' AND category LIKE '%$cat%' ORDER BY saves LIMIT $lowlim, $highlim";
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "good"){
				location.href = 'popular.php?=<?php echo $_SESSION['category']?>';
			} else {
